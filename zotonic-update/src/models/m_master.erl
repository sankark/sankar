...
test() ->
	io:format("test"),
 Props=[{rule_name,"test"},{pattern,"test"},{condition,"Condition"},{action,"Action"},{client_state,"State"}],
 Context=z_context:new(default),
 insert(Props,Context),
	getAll(rules,Context).
...
getAll(Table,Context)->
	 Result = z_db:assoc_props("select * from master",Context),
	Result.

insert(Props, EscapeTexts, Context) ->
    update(insert_rules, Props, EscapeTexts, Context).
...
update(Id, Props, Options, Context) when is_integer(Id) orelse Id == insert_rules ->
    ...
    IsEditable = Id == insert_rsc orelse z_acl:rsc_editable(Id, Context) orelse not(AclCheck),
            ...
            TextProps = recombine_languages(Props, Context),
            ...
            TransactionF = fun(Ctx) ->
            	case Id of
                    insert_rules ->
						    {ok,RowsDeletes} = z_db:delete(master, Ctx),
                            {ok, InsId} = z_db:insert(master, SafeProps, Ctx)
            	end
				end,
