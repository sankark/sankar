...
-module(action_default_proto_edit).
-author("Marc Worrell <marc@worrell.nl").
...
event(#postback{message={predicate_edit, Id, OnSuccess}}, Context) ->
    case z_acl:is_allowed(delete, Id, Context) of
        true ->
             ...
             Result=lists:flatten(m_models:get("models",Id,Context)),
			io:format("After delete ~n~p",[Result]),
    		  {Html, Context1} = z_template:render_to_iolist("_model_form.tpl", [{edit_rule,Result}], Context),
                   z_render:update("model_div", Html, Context1);
